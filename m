...
local function UIOpen()
    ...
    task.spawn(function()
        while task.wait() do
            ...
            if CheckStatusFeature(Func_Other["Auto Collect Fruit"]) == "TRUE" then
                for i,v in pairs(game.Workspace:GetChildren()) do
                    if string.find(v.Name, "Fruit") then
                        InstantTeleport(v.Handle.CFrame)
                    end
                end
            end
        end
    ...
    local SoulReaper = E:AddToggle("Auto Saber", {Title = "Auto Saber", Description = "Legendary Sword (Sea 1)", Default = false})
    SoulReaper:OnChanged(function(v)
    ...
    -- Miscellaneous Tab
    I:AddButton({Title = "X-Ray", Description = "",
        Callback = function()
    ...
    I:AddButton({Title = "Max Zoom", Description = "",
        Callback = function()
